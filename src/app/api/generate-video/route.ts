            ...
            if (videoResponse.ok) {
              ...
              if (videoData.videos && videoData.videos.length > 0) {
                for (const video of videoData.videos) {
                  ...
                  // Select the smallest video file: prefer 'sd' quality, otherwise lowest width
                  const videoFile = videoFiles
                    .filter((f: any) => f.height >= 720)
                    .sort((a: any, b: any) => {
                      // Prefer sd quality first
                      if (a.quality === 'sd' && b.quality !== 'sd') return -1;
                      if (b.quality === 'sd' && a.quality !== 'sd') return 1;
                      // Then sort by lowest width (smallest file)
