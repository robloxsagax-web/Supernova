...
// Vercel serverless function timeout (max 120s on free tier)
...
export async function POST(request: Request) {
  ...
  console.log(`[${requestId}] Script generation request started`);
  ...
  try {
    ...
    console.log(`[${requestId}] Forwarding to FastAPI: ${FASTAPI_URL}/script`);
    ...
    if (!response.ok) {
      let errorDetail = 'Failed to generate script';
    ...
    console.log(`[${requestId}] Script generation successful`);
    ...
    const totalDuration = Date.now() - startTime;
    console.error(`[${requestId}] Script generation proxy error after ${totalDuration}ms:`, error);
